    ...
    <main className="grow">
      ...
      <section className="w-full bg-white py-16 md:py-24">
        <div className="max-w-7xl mx-auto px-5">
          ...
          <p className="font-sans text-lg text-gray-700 text-center mt-12">
            Exhibition opportunities are now on the{" "}
            <Link href="/exhibitors" className="text-primary font-semibold hover:underline">Exhibitors page</Link>.
          </p>
        </div>
      </section>
      ...
      <section className="w-full bg-gray-50 py-16 md:py-24">
        <div className="max-w-7xl mx-auto px-5">
          <div className="text-center mb-14">
            <h2 className="font-heading font-bold text-3xl md:text-4xl text-primary mb-4">Our Sponsors</h2>
            <div className="w-24 h-1 bg-secondary mx-auto mb-6"></div>
            <p className="font-sans text-lg text-gray-700 max-w-3xl mx-auto leading-relaxed">
              We are proud to have the support of organisations committed to advancing Nigeria&apos;s industrial workforce and human capital development.
            </p>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
            {sponsors.map((sponsor) => (
              <div key={sponsor.name} className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col items-center justify-center aspect-[4/3] p-6 hover:shadow-md transition-shadow">
                <div className="w-full h-20 flex items-center justify-center">
                  {sponsor.image ? (
                    <img src={sponsor.image} alt={`${sponsor.name} logo`} className="max-h-full max-w-full object-contain" />
                  ) : (
                    <div className="w-full h-full bg-primary/10 rounded-xl flex items-center justify-center">
                      <span className="font-heading font-bold text-lg text-primary text-center leading-tight px-2">{sponsor.name}</span>
                    </div>
                  )}
                </div>
                <p className="font-sans text-sm text-gray-600 text-center mt-4">{sponsor.name}</p>
              </div>
            ))}
          </div>

          <p className="font-sans text-base text-gray-500 italic text-center mt-10">
            More sponsors will be announced as preparations for NIMS 2026 continue.
          </p>
        </div>
      </section>
      ...
      <section className="w-full py-16 md:py-24 px-5">
        <div className="relative flex flex-col py-20 px-5 md:px-12.5 max-w-7xl mx-auto w-full items-center text-center rounded-[20px] overflow-hidden shadow-xl">
          ...
          <h2 className="font-heading font-bold text-3xl md:text-4xl text-white mb-6 z-10">Become a Sponsor</h2>
          ...
          <p className="font-sans text-lg text-gray-100 leading-relaxed mb-10 max-w-3xl z-10">
            Our team will be happy to discuss sponsorship opportunities and customised partnership options.
          </p>
